...
const one = (first_name) => `Hello ${first_name}`;
...
const three = (text) => text.includes("lemons") ? "Yes" : "No";
...
const six = () => {
  const names = ["Fred", "Mark", "Michelle", "Jack", "Oliver", "Matilda", "Lily"];
  return names.sort();
};
...
const seven = () => {
  ...
  return fruits.filter(fruit => fruit !== "Lemon");
};
...
const nine = () => {
  const personFirstPart = { name: "Julia", age: 20 };
  const personLastPart = { favoriteColor: "orange" };
...
const thirteen = () => {
  const fruits = [{ name: "Orange", color: "orange" }, { name: "Lemon", color: "yellow" }];
...
const fourteen = () => {
  const { color } = { name: "Lemon", color: "yellow" };
  return color;
};
...
const fifteen = () => {
  const numbers = ["Banana", "Orange", "Lemon", "Apple", "Mango"];
