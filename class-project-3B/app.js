...
const three = (text) => {
  ...
  return text.includes("lemons") ? "Yes" : "No";
};
...
const six = () => {
  ...
  return `The color of the lemon is ${lemon.color}`;
};
...
const eight = () => {
  ...
  const numbers = [1, 2, 3, 4, 5];
  return numbers.includes(4) ? "Yes" : "No";
};
...
const eleven = () => {
  ...
  return `Full Name: ${person.first_name} ${person.last_name}`;
};
...
const fourteen = () => {
  ...
  return fruits.filter((fruit) => fruit.name !== "Orange");
  ß;
};
